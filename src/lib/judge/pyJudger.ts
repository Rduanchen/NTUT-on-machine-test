...
// --- 4. 範例執行 (Example Execution) ---

// 為了讓此檔案可以在 Node.js 環境中運行，我們需要一個 __dirname 的定義
// 在 TypeScript 專案中，通常在 tsconfig.json 中設定 "module": "CommonJS" 或使用特定 loader
...
if (require.main === module) {
  ...
  // b = int(input())
  // print(a + b)
  const SCRIPT_PATH: string = path.join(__dirname, "sum.py");

  const testCaseExample2: TestGroup[] = [
    {
      title: "Group1 (公開)",
      id: 1,
      openTestCases: [
  ...
  runPythonTestsAPI(SCRIPT_PATH, testCaseExample2)
    .then((result: RunTestsResult) => {
      console.log("\n======================================");
      console.log("✅ API 執行結果 (新 JSON 格式)：");
      console.log(
        `總測試案例數: ${result.testCaseAmount}, 通過數: ${result.correctCount}`
      );
      console.log("======================================");
      console.log(JSON.stringify(result, null, 2));
    })
    .catch((err: Error) => {
      console.error("API 呼叫發生錯誤:", err);
    });
}
